...
<div class="row">
    <div class="col-8">
        <form action="{{ (isset($tagihan) ? route('tagihan.update', $tagihan->id) : route('tagihan.create')) }}"
            ...
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-12">
                        ...
                        <div class="form-group">
                            ...
                            <div class="custom-switch custom-switch-label-onoff d-flex align-items-center">
                                <input type="checkbox" id="check" name="peserta" value="1" class="custom-switch-input"
                                    {{ isset($tagihan) ? ($tagihan->wajib_semua == 1 ? 'checked' : '') : 'checked' }} >
                            ...
                            <div class="custom-switch custom-switch-label-onoff d-flex align-items-center">
                                <input type="checkbox" id="check1" name="peserta" value="2" class="custom-switch-input"
                                    {{ isset($tagihan) ? (($tagihan->kelas_id != null) ? 'checked' : '') : '' }}>
                            ...
                            <div class="custom-switch custom-switch-label-onoff d-flex align-items-center">
                                <input type="checkbox" id="check2" name="peserta" value="3" class="custom-switch-input"
                                    {{ isset($tagihan) ? (($tagihan->kelas_id == null && $tagihan->wajib_semua == null)
                                ? 'checked' : '') : '' }}>
                        ...
                        <div class="form-group"
                            style="display: {{ isset($tagihan) ? (($tagihan->kelas_id != null) ? 'block' : 'none') : 'none' }}"
                            id="form-kelas">
                            <label class="form-label">Kelas</label>
                            <select class="form-control" name="kelas_id" id="hanya-kelas">
                                @foreach($kelas as $item)
                                <option value="{{ $item->id }}" {{ isset($tagihan) ? (($tagihan->kelas_id == $item->id)
                                    ? 'selected' : '') : '' }}>
                        ...
                        <div class="form-group"
                            style="display: {{ isset($tagihan) ? (($tagihan->kelas_id == null && $tagihan->wajib_semua == null) ? 'block' : 'none') : 'none' }}"
                            id="form-siswa">
                            <label class="form-label">Siswa</label>
                            <select class="form-control" name="siswa_id[]" id="hanya-siswa" multiple>
                                @foreach($siswa as $item)
                                <option value="{{ $item->id }}" {{ isset($tagihan) ? (($tagihan->wajib_semua == null &&
                                    $tagihan->kelas_id == null) ? (in_array($item->id,
                                    $tagihan->siswa->pluck('id')->toArray()) ? 'selected' : '') : '') : '' }}>
                                    {{ $item->nik.'-'.$item->nama }}-{{ $item->kelas->nama }} {{
